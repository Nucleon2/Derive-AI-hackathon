...
export const marketService = {
  calculateReturns(series: Array<[number, number]>): number[] {
  ...
  standardDeviation(values: number[]): number {
    if (values.length === 0) return 0;
    const avg = marketService.average(values);
    const variance = marketService.average(
  ...
  analyzeTechnicalPatterns(
    ...
    const returns = marketService.calculateReturns(recent);
    const avgReturn = marketService.average(returns);
    const volatility = marketService.standardDeviation(returns);
  ...
  buildNewsFromStatusUpdates(coin: CoingeckoCoin): TokenNewsEvent[] {
  ...
  buildSentimentFromCoinData(coin: CoingeckoCoin): TokenSentiment {
    ...
    const aggregate = marketService.average(sources.map((entry) => entry.score));
  ...
  async buildTokenMarketSnapshot(
    ...
    const technicalPatterns = marketService.analyzeTechnicalPatterns(
      prices,
    ...
    return {
      ...
      marketData,
      technicalPatterns,
      news: marketService.buildNewsFromStatusUpdates(coin),
      sentiment: marketService.buildSentimentFromCoinData(coin),
